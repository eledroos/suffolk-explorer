...
SCENARIOS = [
    ...
    ("dispositions lens, review column",
     f"SELECT dtp_review, count(*) FROM {H} WHERE disposed_in_window GROUP BY 1 ORDER BY 1"),
    ...
    ("filings + court=Boston Municipal Court, review column",
     f"SELECT dtp_review, count(*) FROM {H} WHERE filed_in_window AND court='Boston Municipal Court' GROUP BY 1 ORDER BY 1"),
    ...
    ("filings + date range 2024-01-01..2024-12-31, review column",
     f"SELECT dtp_review, count(*) FROM {H} WHERE filed_in_window AND filing_date BETWEEN '2024-01-01' AND '2024-12-31' GROUP BY 1 ORDER BY 1"),
